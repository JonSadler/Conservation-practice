...
# as we logged area 'on the fly' in our modelling we cannot display an X-axis
# using the raw data because that variable doesn't exist in the datafile.
# we need to generate it we create a new vector to use in plot command.


op <- par(mfrow = c(1,2))
plot(Loyn.lm.final$resid ~ log10(Loyn$AREA))	# Looks okay
boxplot(Loyn.lm.final$resid ~ Loyn$GRAZE)	# Again	slight indication of a problem here but it is okay
par(op)
