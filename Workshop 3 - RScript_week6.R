# ************************************************************************************
# This is the source script for Workshop 3 [Workshop 3]: Linear Regression +         *
# model validation                                                                   *
# ************************************************************************************
# Jon Sadler March 8th 2022
...
# Load your libraries
library(tidyverse)
library(ggfortify) # You might need to install this library

#mjm Load compensation.csv datafile from Beckerman and Petchey's book.....see readings
...
# NOTICE HOW MUCH EASIER THIS IS TO DO!
...
# create a sequence of 1000 number spanning the range of humidity (min to max)
x <- seq(min(Flour$HUMIDITY), max(Flour$HUMIDITY), l=1000)  # notice this is an 'l' = length. NOT a 1!!!!!!!
#for each value of x, calculate the upper and lower 95% confidence
y<-predict(Flour.lm, data.frame(HUMIDITY=x), interval="c")
#plot the upper and lower 95% confidence limits
...
ggplotRegression <- function (fit) {
  ...
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) +
    geom_point() + stat_smooth(method = "lm", col = "red") + labs(title = paste("Adj R2 = ",signif(summary(fit)$adj.r.squared, 5),
                       "Intercept =",signif(fit$coef[[1]],5 ),
                       " Slope =",signif(fit$coef[[2]], 5),
                       " P =",signif(summary(fit)$coef[2,4], 5)))}

# NOTE: function only works for linear models without factors! 
#And it uses the adjusted R2 (adj.r.squared)
# It is NOT expected that you understand the function! So please do not worry about it.
...
p + annotate("text", x = 75, y = 9, label = "WEIGHTLOSS = -0.053HUMIDITY + 8.704") +
  annotate("text", x = 62, y = 8.6, label = "R2 = 0.9975")
# NOTE: the X value is the centre of the text!
...
# YOU DO NOT NEED TO UNDERSTAND THE SIMULATION CODE - IT'S A MEANS TO AN END!
# TO GENERATE REDIDUALS THAT ARE PATTERNED!
...
#run using one simple command from ggfortify via ggplot2
autoplot(mussel.lm)
